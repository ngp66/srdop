...
class HTC:
    ...
    def plot_waterfall(self, savefig = False, tf = 101.1, kspace = False, legend = False):
        step = 20*self.dt
        ...
        for i in range(len(slices)):
            ...
            n_L_i = n_L_arr[i*self.Nk:(i+1)*self.Nk]
            ax.plot(self.Ks, n_L_i, label = f't = {slices[i]}', zdir = 'y', zs=slices[i], zorder = (len(slices)-i), color=colors[i])
        if kspace:
            ax.set_xlabel('$k$')
        else:
            ax.set_xlabel('$r_n$')
